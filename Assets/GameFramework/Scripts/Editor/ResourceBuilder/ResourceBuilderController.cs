    ...
    public sealed partial class ResourceBuilderController
    {
        ...
        public string OutputPackagePath
        {
            get
            {
                ...
                return Utility.Path.GetRegularPath(new DirectoryInfo(Utility.Text.Format("{0}/Package/{1}/{2}/{3}_{4}/", 
                    OutputDirectory, ChannelID, Platforms.ToString(), ApplicableGameVersion.Replace('.', '_'), InternalResourceVersion)).FullName);
            }
        }
        ...
        public string OutputFullPath
        {
            get
            {
                ...
                return Utility.Path.GetRegularPath(new DirectoryInfo(Utility.Text.Format("{0}/Full/{1}/{2}/{3}_{4}/",
                    OutputDirectory, ChannelID, Platforms.ToString(), ApplicableGameVersion.Replace('.', '_'), InternalResourceVersion)).FullName);
            }
        }
        ...
        public string OutputPackedPath
        {
            get
            {
                ...
                return Utility.Path.GetRegularPath(new DirectoryInfo(Utility.Text.Format("{0}/Packed/{1}/{2}/{3}_{4}/", 
                    OutputDirectory, ChannelID, Platforms.ToString(), ApplicableGameVersion.Replace('.', '_'), InternalResourceVersion)).FullName);
            }
        }
        ...
        public string BuildReportPath
        {
            get
            {
                ...
                return Utility.Path.GetRegularPath(new DirectoryInfo(Utility.Text.Format("{0}/BuildReport/{1}/{2}/{3}_{4}/",
                    OutputDirectory, ChannelID, Platforms.ToString(), ApplicableGameVersion.Replace('.', '_'), InternalResourceVersion)).FullName);
            }
        }
        ...
        public bool BuildResources()
        {
            ...
            try
            {
                ...
                if (m_BuildEventHandler != null)
                {
                    ...
                    EditorUtility.DisplayProgressBar("预处理资源", "Execute build event handler 'OnPreprocessAllPlatforms", 0.5f);
                    //TODO 打包，这里面填充资源列表： ResourceCollection.xml
                ...
                EditorUtility.DisplayProgressBar("Analyze Dependency", "Start analyze assets dependency", 0.5f);
                ...
                EditorUtility.DisplayProgressBar("PrepareBuildData", "Start prepare build data...", 0.5f);
                ...
                //TODO 打包 准备数据：ABB（AssetBundleBuild)
                ...
                EditorUtility.DisplayProgressBar("一切就绪开始打包", "Start build resources for selected platforms...", 0.5f);
        ...
        private bool BuildResources(Platform platform, AssetBundleBuild[] assetBundleBuildDatas, BuildAssetBundleOptions buildAssetBundleOptions, ResourceData[] assetBundleResourceDatas, ResourceData[] binaryResourceDatas)
        {
            ...
            string outputPackagePath = OutputPackagePath;
            ...
            string outputFullPath = OutputFullPath;
            ...
            string outputPackedPath = OutputPackedPath;
            ...
            m_BuildReport.LogInfo("Unity buildAssetBundleOptions = {0}", buildAssetBundleOptions);
            ...
            for (int i = 0; i < assetBundleResourceDatas.Length; i++)
            {
                ...
                if(i % 100 == 0 )
                    EditorUtility.DisplayProgressBar("Process AssetBundles", fullName, (float)(i + 1) / assetBundleResourceDatas.Length);

                //目前只为Resource Builder工具服务的（然而，PS不在这个工具里执行build操作了）
            ...
            for (int i = 0; i < binaryResourceDatas.Length; i++)
            {
                ...
                if (i % 100 == 0)
                    EditorUtility.DisplayProgressBar("Process Binaries", fullName, (float)(i + 1) / binaryResourceDatas.Length);
            ...
            if (OutputFullSelected)
            {
                ...
                //TODO 打包 这个log，不能随便更改。AssetBundleWebRequest.Send里 会用到。 by李文国
                m_BuildReport.LogInfo("Process updatable version list for '{0}' complete, " +
                                      "updatable version list path is '{1}', " +
                                      "length is '{2}', " +
                                      "hash code is '{3}[0x{3:X8}]', " +
                                      "compressed length is '{4}', " +
                                      "compressed hash code is '{5}[0x{5:X8}]', " +
                                      "channelId is '{6}'.", 
                                    platformName, 
                                    versionListData.Path,
                                    versionListData.Length, 
                                    versionListData.HashCode, 
                                    versionListData.CompressedLength, 
        ...
        private bool ProcessAssetBundle(Platform platform, string workingPath, string outputPackagePath, string outputFullPath, string outputPackedPath, bool zip, string name, string variant, string fileSystem)
        {
            ...
            return ProcessOutput(platform, outputPackagePath, outputFullPath, outputPackedPath, zip, name, variant, fileSystem, resourceData, bytes, length, hashCode, compressedLength, compressedHashCode);
        }
        ...
        private bool ProcessOutput(Platform platform, string outputPackagePath, string outputFullPath, string outputPackedPath, bool zip, string name, string variant, string fileSystem, ResourceData resourceData, byte[] bytes, int length, int hashCode, int compressedLength, int compressedHashCode)
        {
        ...
        private static string GetExtension(ResourceData data)
        {
            ...
            {
                if (data.GetAssetNames().Length == 0)
                {
                    Debug.LogError("PLL:Error GetExtension失败！空包 name = " + data.Name);
