...
namespace UnityGameFramework.Editor.ResourceTools
{
    public sealed class ResourceEditorController
    {
        ...
        private readonly List<string> m_SourceAssetUnSearchPaths; //过滤掉的路径，不扫描
        private readonly List<string> m_SourceAssetSearchRelativePaths;
        private readonly List<string> m_SourceAssetUnSearchRelativePaths;
        private readonly Dictionary<string, SourceAsset> m_SourceAssets;    //扫描GameMain文件夹，得到的所有资源
        private SourceFolder m_SourceAssetRoot;                             //GameMain文件夹
        ...
        public ResourceEditorController()
        {
            ...
            m_SourceAssetUnSearchPaths = new List<string>();
            m_SourceAssetSearchRelativePaths = new List<string>();
            m_SourceAssetUnSearchRelativePaths = new List<string>();
        ...
        public string SourceAssetRootPath
        {
            ...
            set
            {
                ...
                RefreshSourceAssetUnSearchPaths();
            }
        }
            ...
            try
            {
                ...
                for (int i = 0; i < xmlNodeList.Count; i++)
                {
                    ...
                    switch (xmlNode.Name)
                    {
                        ...
                        case "SourceAssetUnSearchPaths":
                            m_SourceAssetUnSearchRelativePaths.Clear();
                            XmlNodeList xmlNodeListTemp = xmlNode.ChildNodes;
                            XmlNode xmlNodeTemp = null;
                            for (int j = 0; j < xmlNodeListTemp.Count; j++)
                            {
                                xmlNodeTemp = xmlNodeListTemp.Item(j);
                                if (xmlNodeTemp.Name != "SourceAssetUnSearchPath")
                                {
                                    continue;
                                }

                                m_SourceAssetUnSearchRelativePaths.Add(xmlNodeTemp.Attributes.GetNamedItem("RelativePath").Value);
                            }
                            break;
        ...
        public bool Save()
        {
            try
            {
                ...
                xmlElement = xmlDocument.CreateElement("SourceAssetUnSearchPaths");
                xmlSettings.AppendChild(xmlElement);
                foreach (string sourceAssetUnSearchRelativePath in m_SourceAssetUnSearchRelativePaths)
                {
                    XmlElement xmlElementInner = xmlDocument.CreateElement("SourceAssetUnSearchPath");
                    xmlAttribute = xmlDocument.CreateAttribute("RelativePath");
                    xmlAttribute.Value = sourceAssetUnSearchRelativePath;
                    xmlElementInner.Attributes.SetNamedItem(xmlAttribute);
                    xmlElement.AppendChild(xmlElementInner);
                }
        ...
        /// <summary>
        /// 获取Resource对象里的所有Asset
        /// </summary>
        /// <param name="name">Resource 名字</param>
        /// <param name="variant">Resource 变体</param>
        /// <returns></returns>
        ...
        public void ScanSourceAssets()
        {
            ...
            foreach (string guid in guids)
            {
                ...
                for (int i = 0; i < m_SourceAssetUnSearchPaths.Count; i++)
                {
                    if (fullPath.Contains(m_SourceAssetUnSearchPaths[i]))
                    {
                        isContinue = true;
                        break;
                    }
        ...
        private void RefreshSourceAssetUnSearchPaths()
        {
            m_SourceAssetUnSearchPaths.Clear();
            ...
            if (m_SourceAssetUnSearchRelativePaths.Count > 0)
            {
                foreach (string sourceAssetUnSearchRelativePath in m_SourceAssetUnSearchRelativePaths)
                {
                    m_SourceAssetUnSearchPaths.Add(Utility.Path.GetRegularPath(Path.Combine(m_SourceAssetRootPath, sourceAssetUnSearchRelativePath)));
                }
            }
        }
