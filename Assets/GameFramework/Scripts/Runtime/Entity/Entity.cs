...
namespace UnityGameFramework.Runtime
{
    ...
    public sealed class Entity : MonoBehaviour, IEntity
    {
        ...
        public void OnLaterUpdate(float elapseSeconds, float realElapseSeconds)
        {
            try
            {
                m_EntityLogic.OnLaterUpdate(elapseSeconds, realElapseSeconds);
            }
            catch (Exception exception)
            {
                Log.Error("Entity '[{0}]{1}' OnLaterUpdate with exception '{2}'.", m_Id, m_EntityAssetName, exception);
            }
        }
    }
}
