...
namespace UnityGameFramework.Runtime
{
    ...
    public sealed class UIForm : MonoBehaviour, IUIForm
    {
        ...
        public void OnGroupHide()
        {
            try
            {
                m_UIFormLogic.OnGroupHide();
            }
            catch (Exception exception)
            {
                Log.Error("UI form '[{0}]{1}' OnGroupHide with exception '{2}'.", m_SerialId, m_UIFormAssetName, exception);
            }
        }
        ...
        /// 界面组显示。
        /// </summary>
        public void OnGroupShow()
        {
            try
            {
                m_UIFormLogic.OnGroupShow();
            }
            catch (Exception exception)
            {
                Log.Error("UI form '[{0}]{1}' OnGroupShow with exception '{2}'.", m_SerialId, m_UIFormAssetName, exception);
            }
        }
