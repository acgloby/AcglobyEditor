...
namespace UnityGameFramework.Runtime
{
    ...
    public sealed class SceneComponent : GameFrameworkComponent
    {
        ...
        public void UnloadScene(string sceneAssetName, object userData)
        {
            ...
            //unloadscene成功后再执行这步， 否则当前激活的场景还是要卸载的场景，会导致后面报错
            //RefreshSceneOrder();
        }
        ...
        {
            Log.Error("Load scene failure, scene asset name '{0}', error message '{1}'.", e.SceneAssetName, e.ErrorMessage);
        ...
        {
            Log.Error("Unload scene failure, scene asset name '{0}'.", e.SceneAssetName);
