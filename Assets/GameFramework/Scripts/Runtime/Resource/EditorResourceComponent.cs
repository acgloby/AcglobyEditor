...
#pragma warning restore 0067, 0414
...
#if UNITY_EDITOR
            string[] dependencyNames = UnityEditor.AssetDatabase.GetDependencies(sceneAssetName, true);
#endif
        }
        ...
        {
            string assetFullName = Application.dataPath.Substring(0, Application.dataPath.Length - AssetsStringLength) + binaryAssetName;
            if (!File.Exists(assetFullName))
            {
                return null;
            }

            using (FileStream fileStream = File.OpenRead(assetFullName))
            {
                fileStream.Position = offset;
                int readLength = (int)fileStream.Length - offset;
                byte[] bytes = new byte[readLength];
                fileStream.Read(bytes, 0, readLength);
                return bytes;
            }
        }
        ...
        public int LoadBinarySegmentFromFileSystem(string binaryAssetName, int offset, byte[] buffer, int startIndex, int length)
        {
            ...
            {
                fileStream.Position = offset;
                return fileStream.Read(buffer, startIndex, length);
            }
        }
