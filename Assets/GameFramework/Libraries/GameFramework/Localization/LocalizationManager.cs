...
namespace GameFramework.Localization
{
    ...
    internal sealed partial class LocalizationManager : GameFrameworkModule, ILocalizationManager
    {
        ...
        // 为优化预加载字典 GCAlloc 的特殊代码
        private const int BufferBytesLength = 1024 * 16;

        private GameFrameworkFunc<string, int> StringToHashFunc;
        private IResourceManager m_ResourceManager;
        private string m_DictionaryAssetName;
        private int m_DictionaryCount;
        private int m_DictionaryTotalCount;
        private byte[] m_CachedBufferBytes;
        private readonly MemoryStream m_CachedMemoryStream;
        private readonly BinaryReader m_CachedBinaryReader;
        ...
        public LocalizationManager()
        {
            ...
            StringToHashFunc = null;
            m_ResourceManager = null;
            m_DictionaryAssetName = null;
            m_DictionaryCount = 0;
            m_DictionaryTotalCount = 0;
            m_CachedBufferBytes = new byte[BufferBytesLength];
            m_CachedMemoryStream = new MemoryStream(m_CachedBufferBytes, true);
            m_CachedBinaryReader = new BinaryReader(m_CachedMemoryStream, Encoding.UTF8);
        }
        ...
        public int DictionaryTotalCount
        {
            get
            {
                return m_DictionaryTotalCount > 0 ? m_DictionaryTotalCount : m_Dictionary.Count;
            }
        }
        ...
        {
            m_CachedBinaryReader.Dispose();
            m_CachedMemoryStream.Dispose();
            m_CachedBufferBytes = null;
        }
        ...
        public void SetStringToHashFunc(GameFrameworkFunc<string, int> stringToHashFunc)
        {
            StringToHashFunc = stringToHashFunc;
        }
        ...
        public void ReadData(string dictionaryAssetName)
        {
            ...
            throw new NotSupportedException("ReadData(string dictionaryAssetName)");
        }
        ...
        public void ReadData(string dictionaryAssetName, int priority)
        {
            ...
            throw new NotSupportedException("ReadData(string dictionaryAssetName, int priority)");
        }
        ...
        public void ReadData(string dictionaryAssetName, object userData)
        {
            ...
            throw new NotSupportedException("ReadData(string dictionaryAssetName, object userData)");
        }
        ...
        public void ReadData(string dictionaryAssetName, int priority, object userData)
        {
            ...
            bool useDictionaryBytes = (bool)userData;
            if (useDictionaryBytes)
            {
                m_DictionaryAssetName = dictionaryAssetName;
                m_DictionaryCount = ReadInt32(0) - 1; // 字典里记录了一条分割符数据，防止 index 访问越界，故实际减 1
                m_DictionaryTotalCount = DictionaryCount + m_DictionaryCount;
            }
            else
            {
                m_DataProvider.ReadData(dictionaryAssetName, priority, userData);
        ...
        public string GetRawString(string key)
        {
            ...
            value = ReadString(offset, GetOffset(index + 1) - offset);
            AddRawString(key, value);
            return value;
        }

        private int FindKeyIndex(string str)
        {
            int hashKey = StringToHashFunc(str);

            int low = 0;
            int high = m_DictionaryCount - 1;
            while (low <= high)
            {
                int middle = (low + high) / 2;
                int middleKey = GetKey(middle);
                if (hashKey == middleKey)
                {
                    return middle;
                }
        ...
        {
            return ReadInt32(GetKeyPosition(index));
        }

        private int GetKeyPosition(int index)
        {
            return sizeof(int) + sizeof(int) * 2 * index;
        }

        private int GetOffset(int index)
        {
            return ReadInt32(GetOffsetPosition(index));
        }

        private int GetOffsetPosition(int index)
        {
            return GetKeyPosition(index) + sizeof(int);
        }

        private int ReadInt32(int offset)
        {
            ReadBytes(offset, sizeof(int));
            return m_CachedBinaryReader.ReadInt32();
        }

        private string ReadString(int offset, int length)
        {
            ReadBytes(offset, length);
            return m_CachedBinaryReader.ReadString();
        }

        private void ReadBytes(int offset, int length)
        {
            if (length > BufferBytesLength)
            {
                throw new GameFrameworkException(Utility.Text.Format("Length '{0}' is too large.", length));
            }

            m_ResourceManager.LoadBinarySegmentFromFileSystem(m_DictionaryAssetName, offset, m_CachedBufferBytes, length);
            m_CachedMemoryStream.Position = 0L;
        }
