Shader "URPDemo/ProceduralSky"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 cloudCoord(half2 uv,half2 offset)
            {
                half4 col = 0;
                for(int i = 1; i <= 2; i++)
                {
                    half4 tex1 = SAMPLE_TEXTURE2D(_CloudNoiseTex1, sampler_CloudNoiseTex1, uv * _CloudNoiseTex1_ST.xy + offset);
                    col = saturate(col + tex1);
                }
                return col;
            }
